...
class U2UCF2():
    def __init__(self, ratings_array) -> None:
        
        self.ratings = {}
        self.item_users = {}
        item_ratings = {}

        for rating_data in ratings_array:
            user_key, item_key, rating = rating_data
            
            user_ratings = self.ratings.get(user_key, {})
            user_ratings[item_key] = rating
            self.ratings[user_key] = user_ratings

            item_rating = item_ratings.get(item_key, 0)
            item_rating += rating
            item_ratings[item_key] = item_rating
            
            item_users = self.item_users.get(item_key, set())
            item_users.add(user_key)
            self.item_users[item_key] = item_users
        
        self.popular_list = sorted(item_ratings.keys(), key = lambda k: item_ratings[k], reverse=True)


    def recommend(self, user_key: int, n_items: int = 20, max_similar_users: int = 1000) -> list:
        user_ratings = self.ratings.get(user_key, {})
        ...
        if similar_users_set.__contains__(user_key):
            similar_users_set.remove(user_key)
